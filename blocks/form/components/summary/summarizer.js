...
class WorkExperienceConverter {
  ...
  static process(result) {
    ...
    let employmentDetails =
      result[WorkExperienceFieldNames.EMPLOYER_NAME].displayValue
    ...
    const newResult = {}
    newResult[WorkExperienceFieldNames.JOB_TITLE] =
      result[WorkExperienceFieldNames.JOB_TITLE]
    newResult.employmentDetails = {
      value: employmentDetails,
      displayValue: employmentDetails,
    }
    newResult[WorkExperienceFieldNames.DESCRIPTION] =
      result[WorkExperienceFieldNames.DESCRIPTION]
...
class EducationConverter {
  ...
  static process(result) {
    ...
    const placeOfLearning =
      result[EducationFieldNames.PLACE_OF_LEARNING]?.displayValue
    ...
    if (completionStatus?.value == 0) {
      ...
      summary.push(
        `${i18n('Started')} ${startYear.displayValue}`,
...
class DriverLicenceConverter {
  static process(result) {
    ...
    newResult[DriverLicenceFieldNames.LICENCE_CLASS] = {
      values: [],
      displayValues: [],
    }
    const hasEndorsements = !isNo(
      result[DriverLicenceFieldNames.ENDORSEMENTS_AVAILABLE],
    )
    if (hasEndorsements) {
    ...
    licenceClass.values.forEach((value, index) => {
      ...
      newResult[DriverLicenceFieldNames.LICENCE_CLASS].values.push(
        `${value}-${stage.value}`,
      )
      newResult[DriverLicenceFieldNames.LICENCE_CLASS].displayValues.push(
...
export class Summarizer {
  // List of converters
  static fieldConverters = [WorkExperienceConverter, EducationConverter]
  ...
  static gotoWizardStep(el) {
    ...
    if (entryId) {
      const editEl = panelEl.querySelector(
        `[data-id="${entryId}"] .repeatable-entry__edit`,
      )
      if (editEl) {
  ...
  static itemContentEditTemplate = `
    <div class="row summary-entry">
        ...
        <div class="col-2">
            <a class="edit" href="#" data-step-name="{{stepName}}" data-entry-id="{{entryId}}">${i18n(
              'Edit',
            )}</a>
        </div>
    </div>
  ...
  static getItemContent(fieldset, stepName, properties) {
    ...
    const template = showEdit
      ? Summarizer.itemContentEditTemplate
      : Summarizer.itemContentTemplate
  ...
  static defaultRepeatableSummarizer(stepName, el, properties) {
    ...
    const entries = form.querySelectorAll(
      `[name="${stepName}"] [data-repeatable].saved`,
    )
  ...
  static personal_statement(el, properties) {
    el.innerHTML = Summarizer.defaultSummarizer(
      'panel_personal_summary',
      el,
      properties,
    )
  }
  ...
  static strengths(el, properties) {
    el.innerHTML = Summarizer.defaultRepeatableSummarizer(
      'panel_strengths',
      el,
      properties,
    )
  ...
  static languages(el, properties) {
    ...
    const englishFieldset = form.querySelector(
      'fieldset[name="panel_english_skills"]',
    )
    const nameValues = {
    ...
    // English content
    let languageContentMarkupObjects =
      Summarizer.markupFromNameValues(nameValues)
    let languageContent = Summarizer.createSummaryFromMarkupObjects(
      languageContentMarkupObjects,
    )
    languageContent = Summarizer.replace(Summarizer.itemContentEditTemplate, {
    ...
    const otherLanguages = form.querySelectorAll(
      'fieldset[name="panel_other_languages"] [data-repeatable].saved',
    )
    properties.showEdit = true
    otherLanguages.forEach((otherLanguage) => {
      languageContent = Summarizer.getItemContent(
        otherLanguage,
        'panel_other_languages',
        properties,
      )
      if (languageContent) {
        languagesContent.push(languageContent)
    ...
    if (languagesContent.length > 0) {
      const content = Summarizer.replace(Summarizer.summaryTemplate, {
        title: properties.title,
        description: properties['description'],
  ...
  static driver_licence(el, properties) {
    ...
    const entry = form.querySelector(
      `[name="${stepName}"] [data-repeatable].saved`,
    )
    const contents = []
  ...
  static work_preferences(el, properties) {
    ...
    el.innerHTML = Summarizer.replace(Summarizer.summaryTemplate, {
      title: properties.title,
      description: properties['description'],
      content: contents.join(''),
    })
