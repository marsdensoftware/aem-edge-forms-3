...
function searchResultsAsRows(jsonResults) {
  return jsonResults.users.map(resultRow => {
    ...
    Object.entries(resultRow).sort((a, b) => a < b ? -1 : a > b ? 1 : 0).forEach(([_, value]) => {
...
async function renderSearch(pager, observer) {
  ...
  await fetch(`https://dummyjson.com/users?${pager.pageSizeArg}=${pager.pageSize}&${pager.offsetArg}=${pager.offset}&select=id,firstName,lastName,age,gender,birthDate,company`).then(r => {
    if (!r.ok) {
