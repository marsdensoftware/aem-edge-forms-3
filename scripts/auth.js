...
const DEFAULT_INFO = Object.freeze({ clientNumber: "", refreshBy: 0 });
...
const userInfo = {
  /** @returns {{clientNumber:string, refreshBy:number}} */
  read() {
...
const createBackend = (origin) => {
  ...
  const decode = async (resp) => {
    ...
    if (ct.includes(MIME.TEXT))
      return { type: "text", data: await resp.text() };
 ...
 * @property {() => {isAuthenticated: boolean, clientNumber: string, refreshBy: number}} status
...
const createAuthClient = ({
  ...
  /* ---- reactive status ---- */
  let status = { isAuthenticated: false, clientNumber: "", refreshBy: 0 };
  const listeners = new Set();

  const updateStatus = (allowChannelUpdate = true) => {
    let { clientNumber, refreshBy } = userInfo.read();
  ...
  return {
    ...
    onAuthenticatedChange: (fn) => listeners.add(fn),
    removeAuthenticatedChange: (fn) => listeners.delete(fn),
...
export const onAuthChange = (fn) => _auth.onAuthenticatedChange(fn);
export const removeAuthChange = (fn) => _auth.removeAuthenticatedChange(fn);
